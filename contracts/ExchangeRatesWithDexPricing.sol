...
contract ExchangeRatesWithDexPricing is ExchangeRates {
    ...
    function effectiveAtomicValueAndRates(
        ...
        if (usePureChainlinkPriceForSource) {
            ...
            sourceRate = _getMinValue(systemSourceRate, _getPriceFromDexAggregator(sourceCurrencyKey, zUSD, sourceAmount));
        }
        ...
        if (usePureChainlinkPriceForDest) {
            ...
            destRate = _getMaxValue(
                ...
                _getPriceFromDexAggregator(zUSD, destinationCurrencyKey, sourceAmount)
            );
        }
    ...
    function _getPriceFromDexAggregator(
        ...
        require(sourceCurrencyKey == zUSD || destCurrencyKey == zUSD, "Atomic swaps must go through zUSD");
        ...
        uint result = _dexPriceDestinationValue(sourceEquivalent, destEquivalent, amount).mul(SafeDecimalMath.unit()).div(
            amount
        );
        require(result != 0, "Result must be greater than 0");

        return destCurrencyKey == "zUSD" ? result : SafeDecimalMath.unit().divideDecimalRound(result);
    }
    ...
    function _dexPriceDestinationValue(
        ...
        uint twapValueInEquivalent = dexPriceAggregator().assetToAsset(
