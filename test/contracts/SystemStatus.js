...
contract('SystemStatus', async accounts => {
	...
	it('not even the owner can suspend', async () => {
		...
		await assert.revert(
			systemStatus.suspendSynthExchange(toBytes32('zBNB'), '1', { from: owner }),
			'Restricted to access control list'
		);
		await assert.revert(
			systemStatus.suspendSynth(toBytes32('zBNB'), '1', { from: owner }),
			'Restricted to access control list'
		);
	});
	...
	describe('when the owner is given access to suspend and resume everything', () => {
		...
		describe('suspendSystem()', () => {
			...
			it('and all the require checks succeed', async () => {
				...
				await systemStatus.requireSynthActive(toBytes32('zBNB'));
				await systemStatus.requireSynthsActive(toBytes32('zBTC'), toBytes32('zBNB'));
			});
			...
			describe('when the owner suspends', () => {
				...
				it('and the require checks all revert as expected', async () => {
					...
					await assert.revert(systemStatus.requireSynthActive(toBytes32('zBNB')), reason);
					await assert.revert(
						systemStatus.requireSynthsActive(toBytes32('zBTC'), toBytes32('zBNB')),
						reason
					);
				});
			});
			...
			describe('when the owner adds an address to suspend only', () => {
				...
				describe('and that address invokes suspend with upgrading', () => {
					...
					it('nor can it do any other restricted action', async () => {
						...
						await assert.revert(
							systemStatus.suspendSynth(toBytes32('zBNB'), '0', { from: account1 })
						);
						await assert.revert(systemStatus.resumeSynth(toBytes32('zBNB'), { from: account1 }));
					});
				...
				describe('when the owner adds an address to resume only', () => {
					...
					describe('and that address invokes resume', () => {
						...
						it('nor can it do any other restricted action', async () => {
							...
							await assert.revert(systemStatus.resumeIssuance({ from: account1 }));
							await assert.revert(
								systemStatus.suspendSynth(toBytes32('zBNB'), '66', { from: account1 })
							);
							await assert.revert(systemStatus.resumeSynth(toBytes32('zBNB'), { from: account1 }));
						});
					});
			...
			describe('when the owner adds an address to suspend only', () => {
				...
				describe('and that address invokes suspend', () => {
					...
					it('nor can it do any other restricted action', async () => {
						...
						await assert.revert(
							systemStatus.suspendSynth(toBytes32('zBNB'), '55', { from: account2 })
						);
						await assert.revert(systemStatus.resumeSynth(toBytes32('zBNB'), { from: account2 }));
					});
					it('yet the owner can still resume', async () => {
				...
				describe('when the owner adds an address to resume only', () => {
					...
					describe('and that address invokes resume', () => {
						...
						it('nor can it do any other restricted action', async () => {
							...
							await assert.revert(systemStatus.resumeSystem({ from: account2 }));
							await assert.revert(
								systemStatus.suspendSynth(toBytes32('zBNB'), '5', { from: account2 })
							);
							await assert.revert(systemStatus.resumeSynth(toBytes32('zBNB'), { from: account2 }));
						});
					});
			...
			describe('when the owner adds an address to suspend only', () => {
				...
				describe('and that address invokes suspend', () => {
					...
					it('but not the others', async () => {
						...
						await systemStatus.requireSynthsActive(toBytes32('zBNB'), sBTC);
					});
					it('and requireExchangeBetweenSynthsAllowed() reverts if one is the given synth', async () => {
						...
						await assert.revert(
							systemStatus.requireExchangeBetweenSynthsAllowed(toBytes32('zBNB'), sBTC),
							reason
						);
						await assert.revert(
							systemStatus.requireExchangeBetweenSynthsAllowed(sBTC, toBytes32('zTRX')),
							reason
						);
						await systemStatus.requireExchangeBetweenSynthsAllowed(
							toBytes32('zBNB'),
				...
				describe('when the owner adds an address to resume only', () => {
					...
					describe('and that address invokes resume', () => {
						...
						it('and all the require checks succeed', async () => {
							await systemStatus.requireSystemActive();
							await systemStatus.requireIssuanceActive();
							await systemStatus.requireExchangeBetweenSynthsAllowed(toBytes32('zBNB'), sBTC);
							await systemStatus.requireSynthActive(sBTC);
							await systemStatus.requireSynthsActive(sBTC, toBytes32('zBNB'));
							await systemStatus.requireSynthsActive(toBytes32('zBNB'), sBTC);
						});
		...
		describe('suspendSynthsExchange()', () => {
			let txn;
			const [sBTC, sETH] = ['zBTC', 'zBNB'].map(toBytes32);
			...
			describe('when the owner adds an address to suspend only', () => {
				...
				describe('and that address invokes suspend', () => {
					...
					it('and requireSynthsActive() reverts if one is the given synth', async () => {
						...
						await assert.revert(systemStatus.requireSynthsActive(toBytes32('zBNB'), sBTC), reason);
						await assert.revert(systemStatus.requireSynthsActive(sBTC, toBytes32('zTRX')), reason);
						await systemStatus.requireSynthsActive(toBytes32('zBNB'), toBytes32('zUSD')); // no issues
