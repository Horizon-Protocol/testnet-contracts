...
function itCanLiquidate({ ctx }) {
	describe('liquidating', () => {
		...
		before('exchange rate is set', async () => {
			exchangeRate = await getRate({ ctx, symbol: 'HZN' });
			await setRate({ ctx, symbol: 'HZN', rate: '1000000000000000000' });
		});

		before('someUser stakes their HZN', async () => {
		...
		describe('getting marked', () => {
			before('exchange rate changes to allow liquidation', async () => {
				await setRate({ ctx, symbol: 'HZN', rate: '200000000000000000' });
			});
			...
			after('restore exchange rate', async () => {
				await setRate({ ctx, symbol: 'HZN', rate: exchangeRate.toString() });
			});
			...
			describe('when the liquidation delay passes', () => {
				...
				describe('getting liquidated', () => {
					...
					before('otherUser calls liquidateDelinquentAccount', async () => {
						beforeDebt = (
							await Synthetix.debtBalanceOf(someUser.address, toBytes32('zUSD'))
						).toString();
